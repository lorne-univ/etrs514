...
def check_content_of_file(file_path, expected_content):
    with open(file_path, "r") as file:
        content = file.read()
        logging.debug("content of {} : {}".format(file_path, content))
...
def check():
    ...
    if os.path.exists(folder):
        ...
        if file_path := os.path.exists(os.path.join(folder, file1)):
            print(f"{colors.get('green')} {file_path} -> OK {colors['reset']}")
            check_content_of_file(
                file_path=file_path, expected_content=expected_content_file1
            )
        if file_path := os.path.exists(os.path.join(folder, file2)):
            print(f"{colors.get('green')} {file_path} -> OK {colors['reset']}")
            check_content_of_file(
                file_path=file_path, expected_content=expected_content_file1
            )
